...
\begin{itemize}
	...
	\itemBold{G14} A taxi driver can logout to the service only when logged in.
	...
	\itemBold{G23} For a registered passenger that has requested a taxi ride, the system allocates a taxi driver to pick him up 10 minutes before the meeting time.
\end{itemize}
...
\begin{itemize}
	...
	\itemBold{UML} Unified Modeling Language.
...
\begin{itemize}
	...
	\itemBold{Server} Also (a bit improperly) referred as \textbf{Back-End}, the centralized part of the system installed in a dedicated facility, that communicates with clients via a network.
	...
	\itemBold{Controller} A piece of software that resides on the server and act as an arbiter between incoming data from the Internet and the application data.
\end{itemize}
