...
Here is presented how the bottom-up integration testing approach is going to be concretely used in \myTaxiService{} system integration, first by analysing each subsystem in detail and then giving a higher level overview on the subsystems integration process.
A precise convention has been adopted for the semantic of diagrams:
\begin{itemize}
	\item
		\textbf{Block}
		\begin{itemize}
			\item \textbf{Yellow:} This block is not dependant on any lower level component in \myTaxiService{} and therefore it is integrated as a starting point.
			\item \textbf{Blue:} This block is going to be integrated on the top of any block that can reach it using a path in the diagram, and depends directly on its parents.
			\item \textbf{Green:} This block can only be il padre di un nodo blu, vuol dire che il blocco e' gia' stato integrato e puo' essere usato per integrarci sopra dei nodi non ancora integrati.
			\item \textbf{marrone:} i blocchi marroni hanno alcune interfacce testate e quindi "verdi" mentre altre disgiunte sono nello stato "blu" e non ancora testate.
			\item \textbf{rosso:} stub
		\end{itemize}
	\item
		freccia == il parent espone un interfaccia usata dal child, quindi percorrere una freccia significa testare se quell'interfaccia specifica esposta dal parent e usata dal child funziona correttamente
\end{itemize}
...
\begin{center}
	\begin{tikzpicture}[node distance=2cm, auto]
		% Place nodes
		\node [block, fill=yellow!50] (datbase_driver_adapter) {Database Driver Adapter};
...
\begin{center}
	\begin{tikzpicture}[node distance=2cm, auto]
		...
		\node [block, left of=query_manager, node distance=3.5cm] (session_manager) {Session Manager};
		...
		\node [block, below of=taxi_sharing_manager, fill=brown!20] (queue_manager) {Queue Manager};
		\node [block, left of=queue_manager, fill=red!20, node distance=3.5cm] (dispatcher_stub) {Dispatcher Stub};
		\node [block, below of=queue_manager, fill=brown!20] (taxi_driver_manager) {Taxi Driver Manager};
		% Draw edges
...
%TODO Questo grafico va spezzato
\begin{center}
	\begin{tikzpicture}[node distance=2cm, auto]
		% Place nodes
		\node [block] (dispatcher) {Dispatcher};
		\node [block, above of=dispatcher, fill=red!20] (td_view_stub) {Taxi Driver View Stub};
		\node [block, below of=dispatcher, fill=red!20] (passenger_view_stub) {Passenger View Stub};
		\node [block, right of=td_view_stub, node distance=4cm, fill=brown!20] (queue_manager) {Queue Manager};
		\node [block, below of=queue_manager, fill=brown!20] (taxi_driver_manager) {Taxi Driver Manager};
		\node [block, below of=taxi_driver_manager, fill=green!20] (session_manager) {Session Manager};
		\node [block, right of=taxi_driver_manager, node distance=4cm] (restful_service) {RESTful Service};
...
\begin{center}
	\begin{tikzpicture}[node distance=4.5cm, auto]
		...
		\node [block, below of=treq, node distance=2cm,, fill=yellow!50] (gps) {GPS Data Source};
