...
\begin{center}
	\begin{tikzpicture}[node distance=2cm, auto]
		...
		\node [block, below of=taxi_ride_manager, fill=brown!20] (taxi_driver_manager) {Taxi Driver Manager};
		\node [block, below of=taxi_driver_manager] (queue_manager) {Queue Manager};
		\node [block, right of=queue_manager, node distance=4cm] (dispatcher) {Dispatcher};
		\node [block, right of=dispatcher, fill=red!20, node distance=4cm] (td_view_stub) {Taxi Driver View Stub};
		\node [block, below of=td_view_stub, fill=red!20] (passenger_view_stub) {Passenger View Stub};
