...
First the \textit{tester} has to test \textit{functionalities} of the components contained in the \textbf{Model Subsystem}, then, in order to simulate the behavior of the \textbf{Interacting Software Parts}, he has to represent the \textbf{Controller Subsystem} with a dedicated \textbf{Driver} in order to test the relevant \textbf{Model} features.
In this way, the \textbf{Model} will be completely integration tested and its behavior is shown by simulating all the \textbf{Controller}'s possible actions.\par
